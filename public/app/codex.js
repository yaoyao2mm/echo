...
export function installCodex(app) {
  ...
  app.renderCodexStatus = function renderCodexStatus(codex) {
    ...
    const agentStatusText = agentOnline ? "本机 agent 在线" : agentAvailable ? "桌面状态同步中" : "等待桌面 agent";
    ...
    state.codexBackendRuntimes = Array.isArray(codex.runtime?.backends) && codex.runtime.backends.length > 0
      ? codex.runtime.backends
      : codex.runtime
        ? [codex.runtime]
        : [];
    const resolvedRuntimePreferences = app.resolveRuntimeBackendChoice(state.runtimePreferences, codex.runtime || state.runtimePreferences);
    app.refreshSelectedBackendRuntime(resolvedRuntimePreferences.backendId || codex.runtime?.backendId || "");
  ...
  app.quickSkillIconMarkup = function quickSkillIconMarkup(skill = {}, usedIconVariants = null) {
    const key = skill.id || [skill.title, skill.description, skill.mode].filter(Boolean).join("|") || "quick-skill";
  ...
  app.fillQuickSkillForm = function fillQuickSkillForm(skill) {
    ...
    if (elements.quickSkillFormTitle) elements.quickSkillFormTitle.textContent = skill.id ? "编辑指令" : "新增指令";
  ...
  app.updateQuickSkillFormControls = function updateQuickSkillFormControls() {
    ...
    app.syncRuntimeSelectControl?.(elements.quickSkillScope);
    app.syncRuntimeSelectControl?.(elements.quickSkillMode);
  };
  ...
  app.updateComposerModeControls = function updateComposerModeControls() {
    ...
    const effectiveEnabled = enabled || planOnly;
    const label = planOnly ? "当前后端权限只允许计划" : enabled ? "退出计划模式" : "进入计划模式";
    elements.composerPlanModeButton.classList.toggle("active", effectiveEnabled);
    elements.composerPlanModeButton.disabled = state.composerBusy || planOnly;
  ...
  app.compactContextUnavailableReason = function compactContextUnavailableReason(session = app.selectedSessionForComposer(), options = {}) {
    ...
    if (attachmentsPending) return "请等待当前图片处理完成";
  ...
  app.composerActionLabel = function composerActionLabel() {
    ...
    if (state.composerPlanMode || app.currentBackendRunsPlanOnly?.()) return "生成计划";
  ...
  app.addComposerAttachmentFiles = async function addComposerAttachmentFiles(files = []) {
    if (!app.currentBackendSupports("attachments")) {
      if (files.length) app.toast("当前后端暂不支持截图附件");
      return;
    }
  ...
  app.updateComposerAvailability = function updateComposerAvailability() {
    ...
    elements.composerAttachmentButton.title = app.currentBackendSupports("attachments") ? "附加截图" : "当前后端暂不支持截图附件";
    ...
    if (elements.compactContextButton) {
      const session = app.selectedSessionForComposer();
      const canCompact = app.canCompactSelectedSession(session);
      elements.compactContextButton.disabled =
        state.composerBusy || attachmentsPending || !commandsAvailable || hasDraft || !canCompact;
      elements.compactContextButton.title = canCompact
        ? "压缩上下文"
        : app.compactContextUnavailableReason(session, { commandsAvailable, attachmentsPending, hasDraft });
    }
    app.updateProjectCreateControls();
