# mobile-agent-transcript-rendering Specification

## Purpose
TBD - created by archiving change improve-agent-transcript-rendering. Update Purpose after archive.
## Requirements
...
- **THEN** transcript 默认显示一行 compact 工具活动摘要和状态
- **AND** 原始命令与 bounded output 仅在用户展开后显示
- **AND** 成功的工具活动默认折叠，失败或等待用户操作的活动可以主动展开
...
- **THEN** transcript 不插入 Git summary part
- **AND** 顶部 Git 状态显示是否有变更
- **AND** 用户展开顶部 Git 状态后可以查看本轮 changed files、branch/ref 和工作区状态中的可用信息
