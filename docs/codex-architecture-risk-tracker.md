...
- [x] Add desktop settings UI for editing the allowed permission mode list.
- [ ] Extend the same desktop-owned policy editing model to every non-Codex backend instead of treating Claude/DeepSeek policy as env-only configuration.
...
- [x] Separate command execution failure from result-reporting failure.
  ...
  - **Fix:** Desktop now retries relay command-completion reporting for retryable relay/network failures instead of converting a completed local command into a failed execution result.

- [x] Make terminal event delivery durable.
  ...
  - **Fix:** Desktop now keeps failed session event posts in a local outbox and retries them, keeping the running heartbeat alive until terminal events such as `turn/completed` and `git.summary` are accepted.

### Medium Priority

- [ ] Replace quick push prompt semantics with structured publish state.
  - **Problem:** The top-right action can ask Codex to commit and push, but the UI state only represents request queue/send completion.
  - **Impact:** The phone can say the action is done while Git push or review handoff is still pending or failed.
  - **Refs:** `public/app/codex.js`, `public/index.html`.
  - **Fix direction:** Have the desktop agent emit structured `git.publish.summary` events with branch, commit, remote, and conclusion.
