...
快速指令把移动端右上角的常用操作扩展成一个可管理的指令入口。它的定位是让手机端快速复用常见 Codex 任务，而不是暴露远程 shell 或任意路径执行能力。
...
- 默认内置一个全局“提交推送”指令，用于提交当前结果并推送到合适的远端分支。
- 手机端可以新增、编辑、删除指令。
- 指令可以选择执行模式：
  - 执行：直接让 Codex 执行任务。
  - 计划：以计划模式发送给 Codex。
- 指令可以标记“需要当前会话”。这类指令必须在一个可继续且没有待处理工作的会话上运行，适合“提交当前结果”“总结当前线程”等依赖上下文的任务。
...
快速指令面板默认提供一个全局“提交推送”指令。这个指令标记为“需要当前会话”，适合在一轮代码改动完成后，直接在同一个移动端会话里整理并发布代码结果。

这个指令只负责 Git 层面的提交和推送，不负责部署服务器或触发仓库维护者自己的发布流水线。默认流程是：

1. 检查 `git status --short --branch`，只处理本次会话相关改动。
2. 根据当前仓库和改动类型运行必要验证。
3. 有可提交改动时创建清晰 commit，然后推送到合适的远端分支。
4. 如果遇到冲突、非快进、权限限制或必须走 PR/CI 审批，停止并说明需要的人工处理。
5. 最后简短汇报验证、commit、push 目标和后续事项。
...
- 默认提交推送指令明确要求不 force push、不绕过分支保护、不提交无关文件。
