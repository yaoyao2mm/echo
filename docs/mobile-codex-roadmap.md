...
- Add retention policies:
  ...
  - max events per job
  - max event age
- Add explicit schema version bookkeeping for SQLite migrations.
- Add relay <-> desktop-agent contract tests for command acquisition, completion, reconnect, and stale lease recovery.
- Keep store refactors behind compatibility-preserving facades so queue and session APIs stay stable.
...
- Validate safety-critical config on startup instead of silently normalizing unexpected values.
- Refresh capability-sensitive presets such as collaboration mode with a TTL or on capability refresh, not only once per process.
...
- Treat the main checkout as a single write slot; parallel write steps must be assigned to isolated worktrees.

### Phone Experience

- A queued job shows whether it will run in the main workspace or an isolated worktree.
- A queued job should also show when it is blocked by an active main-checkout task versus eligible for a worktree-backed slot.
...
The 2026-05 architecture review did not change the product direction, but it surfaced implementation debt that should stay on the roadmap:

- Make main-checkout serialization explicit. Same-project write tasks should queue unless the runtime is backed by an isolated worktree.
- Split `codexStore.js` behind narrower store modules without changing public queue or session semantics.
- Add startup config validation for safety-critical env vars.
- Extract a shared backend adapter base to reduce duplicate capability-probe code.
- Add stable relay <-> desktop-agent contract tests and explicit command/event enums.
- Add schema version bookkeeping for SQLite migrations.
- Put a TTL or refresh trigger on `collaborationModePresets`.
- Keep the frontend stack simple unless a new workflow domain makes a split worthwhile.
...
- Done: add desktop settings UI for allowed permission modes, default permission, worktree mode, worktree root, and retention.
- Done: introduce a desktop backend registry with Codex as the default backend and Claude Code as an opt-in backend.
- Done: support DeepSeek through the Claude Code backend when the Anthropic-compatible base URL points at DeepSeek, including model probing through the DeepSeek models API.
- Done: let the mobile runtime controls follow desktop-advertised backend/model/permission capabilities instead of using one Codex-only model list.
- Done: make the Claude Code backend honor desktop-prepared `execution.path` for isolated worktree sessions and emit Git summaries in the same `git.summary` shape as Codex.
- Done: add `echo.backend-adapter.v1` as an explicit backend compatibility contract for adapter methods, runtime command handling, cancellation, capability flags, unsupported feature reporting, worktree/Git summary expectations, and backend health.
- Done: make the desktop backend registry enforce the contract when registering adapters, creating runtimes, and publishing backend snapshots.
- Done: preserve backend contract metadata through relay storage/status snapshots so mobile and future orchestration code can see backend capabilities and health.
- Done: extract shared backend adapter capability refresh, health, and snapshot normalization into `BaseBackendAdapter`, leaving Codex and Claude Code as thin backend-specific bindings.
- Done: add `ECHO_AGENT_BACKENDS_JSON` for extra Claude Code / Anthropic-compatible backend profiles, so one desktop can advertise multiple backend ids with different commands, model lists, endpoints, and permission defaults.
- Done: add explicit mobile worktree apply/discard actions backed by desktop-executed session commands.
- Done: add a desktop session-event retry outbox so terminal events and Git summaries survive relay/network interruptions.

Next build order:

1. Make execution-slot semantics explicit: same-project main-checkout tasks serialize, and the mobile UI should explain when a queued job is waiting on an active checkout versus eligible for an isolated worktree.
2. Normalize non-Codex backend events and results so mobile and memory code can consume the same `git.summary`, `turn.*`, `approval.*`, `interaction.*`, and `artifact.*` shape regardless of provider.
3. Keep backend/worktree compatibility as a contract test for every backend; writable execution must run inside desktop-prepared `execution.path` when worktree mode is active.
4. Expand backend health/doctor checks beyond static contract health: Codex app-server probe, Claude command/version/auth check, DeepSeek model probe status, and clearer desktop/mobile diagnostics.
5. Add relay <-> desktop-agent contract tests and schema-versioned migration bookkeeping.
6. Improve mobile job detail and timeline for worktree metadata: base branch, base commit, worktree path, result branch, changed files, and cleanup state.
7. Add merge/commit/publish flows for worktree jobs after apply/discard has proven stable.
8. Add broader log masking, raw-event retention, and artifact retention controls.
9. Promote the next orchestration layer: `run` / `step` records, strategy selection, role assignment, and selection reasons.
