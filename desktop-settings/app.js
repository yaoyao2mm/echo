...
function renderWorkbench(health) {
  ...
  headerCopy.append(
    element("h3", "", agentOnline ? "本机 agent 在线" : relay.ok ? "等待桌面 agent" : "Relay 未连接"),
    element("p", "", relay.detail || "桌面端会在这里显示真实的 agent/backend 运行状态。")
  );
  ...
  metrics.append(
    ...
    metricCard("Backends", backends.length || (runtime.backendId ? 1 : 0), backendRosterLabel(backends, runtime)),
  ...
  grid.append(
    ...
    workbenchPanel("Backends", renderRuntime(runtime, health)),
...
function renderCurrentSession(session) {
  if (!session) return emptyState("还没有正在执行或最近的 agent 会话。");
...
function renderRuntime(runtime, health) {
  const localCodex = health?.codex || {};
  const backends = runtimeBackends(runtime);
  if (backends.length > 1) {
    return detailList(
      backends.map((backend) => [
        backend.backendName || backend.backendId || "backend",
        [
          backend.health?.state || (backend.command ? "ready" : "missing command"),
          backend.model || "backend default",
          `permissions ${(backend.allowedPermissionModes || []).join(", ") || "none"}`,
          `supports ${supportedFeatureLabel(backend)}`
        ].join(" · ")
      ])
    );
  }
  ...
  return detailList([
    ...
    ["model", runtime.model || "backend default"],
    ["reasoning", runtime.reasoningEffort || "backend default"],
...
function runtimeBackends(runtime = {}) {
  const backends = Array.isArray(runtime.backends) ? runtime.backends.filter(Boolean) : [];
  if (backends.length) return backends;
  return runtime.backendId || runtime.provider || runtime.backendName ? [runtime] : [];
}

function backendRosterLabel(backends = [], runtime = {}) {
  const items = backends.length ? backends : runtimeBackends(runtime);
  if (!items.length) return "none";
  return items
    .slice(0, 3)
    .map((backend) => backend.backendName || backend.backendId || "backend")
    .join(", ");
}

function supportedFeatureLabel(backend = {}) {
  const supports = backend.capabilities?.supports || {};
  const names = [];
  if (supports.attachments) names.push("attachments");
  if (supports.compaction) names.push("compaction");
  if (supports.approvalRequests) names.push("approvals");
  if (supports.interactionRequests) names.push("interactions");
  if (supports.worktree) names.push("worktree");
  return names.length ? names.join(", ") : "basic turns";
}
...
function renderHealth(health) {
  ...
  const items = [
    ...
    },
    {
      ...
      detail: [health.claude?.path || health.claude?.command, health.claude?.version, health.claude?.detail].filter(Boolean).join("\n")
    },
    {
      title: "Workspaces",
...
function renderCodexDetection(health) {
  ...
  codexDetectedPath.textContent = detail || health.codex?.detail || "未检测到可用命令。";
}
...
function formatState(state) {
  ...
  const lines = [
    ...
    `codex backend: ${valueOf(fields, "ECHO_CODEX_ENABLED") || "true"}`,
    `claude backend: ${valueOf(fields, "ECHO_CLAUDE_ENABLED") || "false"}`,
...
function validateWorkspaceRows() {
  ...
  if (!rows.length) return { ok: false, message: "请至少添加一个工程目录。" };
