...
async function loginToWorkbench(page) {
  ...
  const topbarIconLayout = await page.evaluate(() => {
    ...
    const quickSkills = document.querySelector("#quickSkillsButton")?.getBoundingClientRect();
    ...
    return {
      ...
      quickSkillsInTopbar: Boolean(context && quickSkills && composer && quickSkills.top < composer.top),
      quickSkillsRightOfContext: Boolean(context && quickSkills && quickSkills.left > context.right)
    };
  });
  expect(topbarIconLayout.statusHidden).toBeTruthy();
  expect(topbarIconLayout.quickSkillsInTopbar).toBeTruthy();
  expect(topbarIconLayout.quickSkillsRightOfContext).toBeTruthy();
...
test("mobile composer defaults to the desktop model and remembers the last chosen model", async ({ page, request }) => {
  await touchMockAgent(request);
  const keepAlive = setInterval(() => {
    touchMockAgent(request).catch(() => {});
  }, 10000);
...
test("mobile quick skill sends the fixed commit-and-push prompt", async ({ page, request }) => {
  await touchMockAgent(request);
  const headers = await authHeadersForSessionRequests(request);
  const suffix = Date.now();
  const prompt = `一键提交推送当前对话 ${suffix}`;
  ...
  try {
    ...
    await page.locator(".quick-skill-item", { hasText: "提交推送" }).locator(".quick-skill-run").click();
    ...
    expect(
      sessionData.session.messages.some(
        (message) => message.role === "user" && String(message.text || "").includes("根据当前仓库和改动类型选择必要且可运行的验证")
      )
    ).toBeTruthy();
    expect(sessionData.session.messages.some((message) => /Actions|workflow/.test(String(message.text || "")))).toBeFalsy();
  } finally {
    clearInterval(keepAlive);
  }
});
