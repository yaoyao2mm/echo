...
test("resolveCodexCommand prefers the bundled macOS Codex app when the command is generic", () => {
  const existsSync = (value) => value === "/Applications/ChatGPT.app/Contents/Resources/codex";
  assert.equal(
...
test("resolveDesktopCodexCommand on macOS ignores a legacy brew command and uses Codex (ChatGPT.app)", () => {
  const existsSync = (value) => value === "/Applications/ChatGPT.app/Contents/Resources/codex";
  ...
  assert.equal(result.command, "/Applications/ChatGPT.app/Contents/Resources/codex");
...
test("resolveDesktopCodexCommand on macOS requires the official Codex app", () => {
  ...
  assert.match(result.detail, /ChatGPT\.app.*is required on macOS/);
});
